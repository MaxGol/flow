---
description: Implement the next pending task. Reads TASKS, spawns the implementation agent, archives the result, marks the task done.
---
...
Create `.flow/CURRENT.md` from scratch, describing ONLY the task you picked.
...
> The context file is `.flow/CURRENT.md` — read it, then follow your instructions.

Wait for the sub-agent to finish. It will have written its results into the
`## Implementation Log` section of `.flow/CURRENT.md`.
...
2. Copy the completed `.flow/CURRENT.md` to `.flow/archive/CURRENT-[task id].md`
   (e.g. `.flow/archive/CURRENT-M1-1.md`). This is the permanent record of
   this task.
3. Reset `.flow/CURRENT.md` back to a clean, empty state by writing exactly:
...
This keeps `CURRENT.md` clean at rest — the real records live in `.flow/archive/`.
...
- Every task is archived to `.flow/archive/` as its own file. `CURRENT.md` is
